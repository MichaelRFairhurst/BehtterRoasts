...
class RoastSummaryService {
  ...
  RoastSummary summarize(Roast roast, Bean bean) {
	...
	final firstCrackLog = roast.phaseLogs.firstWhere((p) => p.phase == Phase.firstCrack);
	final firstCrackEndLog = roast.phaseLogs.lastWhere((p) => p.phase == Phase.firstCrack);
    ...
    if (roast.phaseLogs.any((p) => p.phase == Phase.secondCrack)) {
      secondCrackPhaseTime = roast.phaseLogs.singleWhere(
		  (p) => p.phase == Phase.secondCrack).time;
